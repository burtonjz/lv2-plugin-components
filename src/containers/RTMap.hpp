 ...
 *  retrieved based on a key. RTMap has got a fixed maximal size.
 *
 ...
 *  For a @c RTMap<Key,T> the key_type is Key, the mapped_type is T, and the
 *  value_type is std::pair<const Key,T>.
 *
 *  RTMaps support random access iterators as std::array did.
 */
template <typename _Key, typename _Tp, std::size_t _Nm>
class RTMap : protected std::array <std::pair<_Key, _Tp>, _Nm>
{
public:
...
public:
    ...
    /**
     *  @brief  Indirect access to %RTMap data via the subscript ( @c [] ) 
     *          and a key.
    ...
    /**
     *  @brief Attempts to insert a std::pair into the %RTMap.
    ...
    /**
     *  @brief Erases an element from a %RTMap.
    ...
    /**
     *  Erases all elements in a %RTMap simply by setting the size to 0.
     */
    void clear() noexcept
    {
        _err = RTMAP_NO_ERROR;
    ...
    // lookup
    /**
     *  @brief Tries to locate an element in a %RTMap.
